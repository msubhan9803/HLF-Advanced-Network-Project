...
addTLSToSystemChannel() {
    ...
    echo "{\"client_tls_cert\":\"$(cat $TLS_FILE | base64 -w 0)\",\"host\":\"orderer4.example.com\",\"port\":10050,\"server_tls_cert\":\"$(cat $TLS_FILE | base64 -w 0)\"}" >org4consenter.json
    jq ".channel_group.groups.Orderer.values.ConsensusType.value.metadata.consenters += [$(cat org4consenter.json)]" config.json >modified_config.json
    ...
    peer channel update -f config_update_in_envelope.pb -c $SYSTEM_CHANNEL_NAME -o localhost:7050 --tls --cafile $ORDERER_CA
}
